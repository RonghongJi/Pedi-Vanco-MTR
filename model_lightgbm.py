# -*- coding: utf-8 -*-
"""
Created on 2026/04/24 13:58:57
...
def grid_search_cv_single_param(param_grid, X, y, fixed_params):
    model = lgb.LGBMRegressor(random_state=seed, **fixed_params)
...
best_params_df.to_excel(r'../output/param/LightGBM_best_params.xlsx', index=False)
best_params_df

# %%
# 使用逐个优化后的最佳参数训练模型
lgb_best_model = lgb.LGBMRegressor(**best_params, random_state=seed)
multi_output_model = MultiOutputRegressor(lgb_best_model)
...
# 计算十折交叉验证的回归指标
lgb_cv_metrics = cross_val_regression_metrics(multi_output_model, X_train, y_train, kf)

# 训练最终模型并预测
multi_output_model.fit(X_train, y_train)
lgb_y_pred = multi_output_model.predict(X_test)

# 计算测试集上的回归指标
lgb_test_metrics = calculate_regression_metrics(y_test, lgb_y_pred)

# 合并十折交叉验证和测试集的回归指标
metrics = {}
for col in y_train.columns:
    metrics[f'Ten_fold_CV_RMSE_{col}'] = f"{lgb_cv_metrics[col]['RMSE'][0]:.2f} ± {lgb_cv_metrics[col]['RMSE'][1]:.2f}"
    metrics[f'Ten_fold_CV_R2_{col}'] = f"{lgb_cv_metrics[col]['R2'][0]:.2f} ± {lgb_cv_metrics[col]['R2'][1]:.2f}"
    metrics[f'Ten_fold_CV_MAE_{col}'] = f"{lgb_cv_metrics[col]['MAE'][0]:.2f} ± {lgb_cv_metrics[col]['MAE'][1]:.2f}"

# 添加测试集指标
for col in y_test.columns:
    metrics[f'Test_RMSE_{col}'] = f"{lgb_test_metrics[col]['RMSE']:.2f}"
    metrics[f'Test_R2_{col}'] = f"{lgb_test_metrics[col]['R2']:.2f}"
    metrics[f'Test_MAE_{col}'] = f"{lgb_test_metrics[col]['MAE']:.2f}"

# 保存到 XLSX 文件
metrics_df = pd.DataFrame(metrics, index=[0])
metrics_df.to_excel(r'../output/ML输出/LightGBM_GridSearch_Regression.xlsx', index=False)
metrics_df
...
lgb_y_pred_df = pd.DataFrame(lgb_y_pred).reset_index(drop=True)  # 同样处理预测值

# 确保列名称正确
y_test_df.columns = ['True Values cl', 'True Values v']
lgb_y_pred_df.columns = ['Predicted Values cl', 'Predicted Values v']

# 将两者合并到一个 DataFrame 中
output_df = pd.concat([y_test_df, lgb_y_pred_df], axis=1)

# 保存到 Excel 文件
output_df.to_excel(r'../output/pred/LightGBM_predictions.xlsx', index=False)
...
for pct in [10, 20, 30, 40, 50]:
    accuracies[f"Accuracy_within_{pct}%_CL"] = calculate_accuracy_within_percentage(y_test['CL'].values, lgb_y_pred[:, 0], pct)
    accuracies[f"Accuracy_within_{pct}%_V"] = calculate_accuracy_within_percentage(y_test['V'].values, lgb_y_pred[:, 1], pct)
...
# 保存到 XLSX 文件
accuracy_df.to_excel(r'../output/accuracy/LightGBM_acc.xlsx', index=False)
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.title('LightGBM Fitted Scatter Plot for CL', fontsize=20)
plt.show()
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.title('LightGBM Fitted Scatter Plot for V', fontsize=20)
plt.show()

# 绘制残差图
plt.figure(figsize=(10, 6))
residuals_cl = y_test['CL'] - lgb_y_pred[:, 0]
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.title('LightGBM Residual Plot for CL', fontsize=20)
plt.show()

plt.figure(figsize=(10, 6))
residuals_v = y_test['V'] - lgb_y_pred[:, 1]
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.title('LightGBM Residual Plot for V', fontsize=20)
plt.show()
